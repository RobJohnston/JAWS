...
function refreshWhitelistAddDropdown(crew) {
    ...
    // Remove all options except the placeholder and "All boats"
...
async function handleWhitelistAdd() {
    ...
    try {
        // Handle "All boats" option
        if (selectedValue === '__ALL__') {
            // Add all available boats to whitelist
            const whitelist = targetUserData.crew.whitelist || [];
            const availableBoats = allBoats.filter(b => !whitelist.includes(b.key));
            
            // Add each boat sequentially
            for (const boat of availableBoats) {
                const updated = await adminService.addToCrewWhitelist(targetUserData.crew.key, boat.key);
                targetUserData.crew = updated;
            }
            
            refreshWhitelistTable(targetUserData.crew);
            refreshWhitelistAddDropdown(targetUserData.crew);
            showToast(`All boats (${availableBoats.length}) added to whitelist.`, 'success');
        } else {
            // Add single boat
            const updated = await adminService.addToCrewWhitelist(targetUserData.crew.key, selectedValue);
            targetUserData.crew = updated;
